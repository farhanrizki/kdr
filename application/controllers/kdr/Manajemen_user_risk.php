...
class Manajemen_user_risk extends CI_Controller{
    var $data_array;
    ...
    public function index()
    {
        ...
        else
        {
            ...
            $data['user_risk_aktif']    = $this->m_kdr->user_risk_aktif();
            $data['user_risk_nonaktif'] = $this->m_kdr->user_risk_nonaktif();
            $this->load->view('kdr/v_header_kdr', $data);
            $this->load->view('kdr/v_manajemenUserRISK', $data); 
    ...
    public function tambah_user_risk()
    {
        ...
        if(!$cek_user)
        {
            $simpan = $this->m_kdr->tambah_user_risk($nama_lengkap,$username,$password,$level,$status,$nama_bagian,$untuk_web);
            if($simpan)
            {
                $data['user_risk_aktif']    = $this->m_kdr->user_risk_aktif();
                $data['user_risk_nonaktif'] = $this->m_kdr->user_risk_nonaktif();
    ...
    public function update_user_aktif()
    {
        ...
        for ($i = 0; $i < count($id_user); $i++) 
        {
            ...
            $update      = $this->m_kdr->update_user_risk_aktif($id,$nama_lengkap,$level,$status,$nama_bagian,
    ...
    {
        $id_user                    = $this->input->post('id_user');
        $hapus                      = $this->m_kdr->hapus_user_risk_aktif($id_user);
        $data                       = $this->data_array;
        $data['user_risk_aktif']    = $this->m_kdr->user_risk_aktif();
        $data['user_risk_nonaktif'] = $this->m_kdr->user_risk_nonaktif();
        $this->load->view('kdr/v_header_kdr', $data);
    ...
    public function update_user_nonaktif()
    {
        ...
        for ($i = 0; $i < count($id_user); $i++) 
        {
            ...
            $nama_bagian = $nama_bagian[$id];
            $untuk_web   = $untuk_web[$id]; 
            $update      = $this->m_kdr->update_user_risk_nonaktif($id,$nama_lengkap,$level,$status,$nama_bagian,
                           $untuk_web,$password_baru,$password_encrypt);
        }
    ...
    {
        $id_user                    = $this->input->post('id_user');
        $hapus                      = $this->m_kdr->hapus_user_risk_nonaktif($id_user);
        $data                       = $this->data_array;
        $data['user_risk_aktif']    = $this->m_kdr->user_risk_aktif();
        $data['user_risk_nonaktif'] = $this->m_kdr->user_risk_nonaktif();
        $this->load->view('kdr/v_header_kdr', $data);
