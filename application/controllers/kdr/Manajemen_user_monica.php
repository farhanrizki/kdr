...
class Manajemen_user_monica extends CI_Controller{
    var $data_array;
    ...
    public function index()
    {
        ...
        else
        {
            ...
            $data['user_monica_aktif']    = $this->m_kdr->user_monica_aktif();
            $data['user_monica_nonaktif'] = $this->m_kdr->user_monica_nonaktif();
            $this->load->view('kdr/v_header_kdr', $data);
            $this->load->view('kdr/v_manajemenUserMONICA', $data); 
    ...
    public function tambah_user_monica()
    {
        ...
        if(!$cek_user)
        {
            $simpan = $this->m_kdr->tambah_user_monica($nama_lengkap,$username,$password,$level,$status,$nama_bagian,
                      $untuk_web);
            if($simpan)
            {
                $data['user_monica_aktif']    = $this->m_kdr->user_monica_aktif();
    ...
    public function update_user_aktif()
    {
        ...
        for ($i = 0; $i < count($id_user); $i++) 
        {
            ...
            $update      = $this->m_kdr->update_user_monica_aktif($id,$nama_lengkap,$level,$status,$nama_bagian,
    ...
    public function update_user_nonaktif()
    {
        ...
        for ($i = 0; $i < count($id_user); $i++) 
        {
            ...
            $nama_bagian = $nama_bagian[$id];
            $untuk_web   = $untuk_web[$id]; 
            $update      = $this->m_kdr->update_user_monica_nonaktif($id,$nama_lengkap,$level,$status,$nama_bagian,
                           $untuk_web,$password_baru,$password_encrypt);
        }
    ...
    {
        $id_user                      = $this->input->post('id_user');
        $hapus                        = $this->m_kdr->hapus_user_monica_nonaktif($id_user);
        $data                         = $this->data_array;
        $data['user_monica_aktif']    = $this->m_kdr->user_monica_aktif();
        $data['user_monica_nonaktif'] = $this->m_kdr->user_monica_nonaktif();
        $this->load->view('kdr/v_header_kdr', $data);
