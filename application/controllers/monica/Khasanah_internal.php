...
class Khasanah_internal extends CI_Controller{
    var $data_array;
    ...
    function __construct()
    {
        ...
        $config['upload_path']   = './upload/compliance/internal/';
    ...
    {
    	if(isset($_POST['tambahkhasanahinternal'])){
            $this->tambah_khasanah_internal();
        }else if(isset($_POST['hapuskhasanahinternal'])){
            $this->hapus_khasanah_internal();
        }else{
        	$data = $this->data_array;
			$data['data_khasanah_internal'] = $this->m_khasanah->data_khasanah_internal();
			$this->load->view('monica/v_header_monica', $data);
			$this->load->view('monica/v_khasanahInternal', $data); 
    ...
    {    
		...
		$fileInfo      = $this->m_khasanah->download_khasanah_internal(array('id_compliance' => $id_compliance));
		$file_name     = $fileInfo['file'];
		$file          = 'upload/compliance/internal/'.$file_name;
        force_download($file, NULL);
        //header('Content-Type: application/pdf');
        //readfile($file);
    } 

    public function detail_khasanah_internal()
    {
		$id_compliance = $this->input->post('id_compliance');
		$result_html   = '';
		$result_set    = $this->m_khasanah->detail_khasanah_internal($id_compliance);
		$today         = date("Y-m-d");
    ...
    public function tambah_khasanah_internal()
    {
        ...
        if (!empty($_FILES['file']['name'])) {
            $path = './upload/compliance/internal/'.$upload_file;
            if(file_exists($path)){
                echo '<script language="javascript">';
                echo 'alert("File yang di upload sudah ada, Data gagal disimpan!!!")';
                echo '</script>';
                redirect('monica/khasanah_internal', 'refresh');
            }else{
        ...
        $simpan = $this->m_khasanah->tambah_khasanah_internal($no_doc,$judul,$deskripsi,$request_oleh,$catatan_khusus,$file_upload,$tipe,
        ...
        if($simpan){
            ...
            $data['berhasil_simpan'] = '<div class="alert alert-success alert-dismissable">
                                            ...
                                            <p style="font-weight:bold;">Khasanah Internal berhasil ditambahkan</p>
                                        </div>';
            $this->load->view('monica/v_khasanahInternal', $data);
        }else{
            $data['gagal_simpan'] = '<div class="alert alert-danger alert-dismissable">
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        <p style="font-weight:bold;">Khasanah Internal gagal ditambahkan</p>
                                    </div>';
            $this->load->view('monica/v_khasanahInternal', $data);
        }
        $this->load->view('monica/v_footer_monica');
    ...
    public function edit_khasanah_internal($id_compliance)
    {
        $data  = $this->data_array; 
        ...
        {
            $data['lihat_internal'] = $this->m_khasanah->lihat_internal($id_compliance);
            $this->load->view('monica/v_header_monica', $data);
            $this->load->view('monica/v_editKhasanahInternal', $data); 
            $this->load->view('monica/v_footer_monica');
        }
        else
        {
    ...
    public function update_khasanah_internal()
    {
        $id_compliance = $this->input->post('id_compliance');
        ...
        if (!empty($_FILES['file']['name'])) {
            $path = './upload/compliance/internal/'.$file_upload;
            if(file_exists($path)){
                echo '<script language="javascript">';
                ...
                if(empty($file_sebelum)){
                    ...
                    $path = './upload/compliance/internal/'.$file_sebelum;
                    unlink($path);
                    $file_upload = $file_upload;
                }
            }
        ...
        $update = $this->m_khasanah->update_khasanah_internal($id_compliance,$no_doc,$judul,$deskripsi,$request_oleh,$catatan_khusus,$file_upload,
                  $diedit_oleh,$diedit_tanggal);

        if($update){
            $upload = $this->upload->do_upload('file');
            echo '<script language="javascript">';
            echo 'alert("Khasanah internal berhasil diupdate")';
            echo '</script>';
            redirect('monica/khasanah_internal', 'refresh');
        }else{
            echo '<script language="javascript">';
            echo 'alert("Khasanah internal gagal diupdate")';
            echo '</script>';
            redirect('monica/khasanah_internal', 'refresh');
        }
    }

    public function hapus_khasanah_internal()
    {
		$id_compliance                  = $this->input->post('id_compliance');
		$file_sebelum                   = $this->input->post('file_sebelum');
		$hapus                          = $this->m_khasanah->hapus_khasanah_internal($id_compliance);
		$data                           = $this->data_array;
		$data['data_khasanah_internal'] = $this->m_khasanah->data_khasanah_internal();
        ...
        if($hapus)
        {
            if(!empty($file_sebelum))
            {
                $path_to_file = './upload/compliance/internal/'.$file_sebelum;
    ...
    public function doc_int_input()
    {
        $no_doc    = $this->input->post('no_doc');
        $tahun     = $this->input->post('tahun');
        $nomor_doc = $no_doc.'-'.$tahun.'-';
        $data      = $this->m_khasanah->doc_int_input($nomor_doc);
        echo json_encode($data);
    }

    public function doc_int_edit()
    {
        $no_doc         = $this->input->post('no_doc');
        $tahun          = $this->input->post('tahun');
        ...
        $nomor_doc      = $no_doc.'-'.$tahun.'-';
        $data           = $this->m_khasanah->doc_int_edit($nomor_doc,$nodoc_terakhir,$angka_terakhir);
        echo json_encode($data);
    }
