...
class Data_audit_internal extends CI_Controller{
    var $data_array;

    function __construct(){
        parent::__construct();
        $this->load->model('monica/m_audit');
        $config['upload_path']   = './upload/audit/internal/';
        ...
        $nama_divisi        = $this->m_audit->nama_divisi();
        $nama_kategori      = $this->m_audit->nama_kategori();
        $risk_issue         = $this->m_audit->risk_issue();
        $proses_major       = $this->m_audit->proses_major();
        $sub_proses         = $this->m_audit->sub_proses();
        if($level_kdr == "adminkdr" || $level_kdr == "staffkdr" || $level_kdr == "kabagkdr" || $level_kdr == "nonadmin"){
            ...
            $tahun_audit_internal = $this->m_audit->tahun_audit_internal($nama_bagian_monica);
            $this->data_array = array(
                ...
                'tahun_audit_internal' => $tahun_audit_internal,
    ...
    public function index(){
        if(isset($_POST['filter'])){
            $this->filter();
        }else if(isset($_POST['tambahauditinternal'])){
            $this->tambah_audit_internal();
        }else if(isset($_POST['updateauditinternal'])){
            $this->update_audit_internal();
        }else if(isset($_POST['hapusauditinternal'])){
            $this->hapus_audit_internal();
        }else{
            ...
            $data['data_audit_internal'] = $this->m_audit->data_audit_internal($tahun,$nama_bagian,$status);
			$this->load->view('monica/v_header_monica', $data);
			$this->load->view('monica/v_dataAuditInternal', $data); 
    ...
    public function lihat_data_langsung($id_audit){
        $data                        = $this->data_array;
        $tahun                       = "";
        $status                      = "";
        $data['tahun']               = $tahun;
        ...
        $nama_bagian                 = $data['nama_bagian_monica'];
        $data['nama_bagian']         = $nama_bagian;
        $data['data_audit_internal'] = $this->m_audit->tl_internal_langsung($id_audit);
        $this->load->view('monica/v_header_monica', $data);
        $this->load->view('monica/v_dataAuditInternal', $data); 
        $this->load->view('monica/v_footer_monica');
    }
	...
	public function download_file(){    
        ...
        $fileInfo  = $this->m_audit->download_file_internal(array('id_audit' => $id_audit));
        $file_name = $fileInfo['upload'];
        $file      = 'upload/audit/internal/'.$file_name;
        force_download($file, NULL);
    } 

    public function download_audit_internal($tahun,$nama_bagian,$status){
        if($status == "semua"){
            $stat = "";
        }else{
            if($status == "memadai"){
        ...
        if($tah == ""){
            if($stat == "")
            {
                $data['isiexcel'] = $this->m_audit->data_audit_internal($tah,$nama_bagian,$stat);
                $data['judul']    = 'Data Audit Internal Div. APP';
            }else{
                $data['isiexcel'] = $this->m_audit->data_audit_internal($tah,$nama_bagian,$stat);
                $data['judul']    = 'Data Audit Internal Div. APP dengan Status Monitoring '.$judul;
            }
        }else{
            if($stat == ""){
                $data['isiexcel'] = $this->m_audit->data_audit_internal($tah,$nama_bagian,$stat);
                $data['judul']    = 'Data Audit Internal Div. APP Tahun '.$tah;
            }else{
                $data['isiexcel'] = $this->m_audit->data_audit_internal($tah,$nama_bagian,$stat);
                $data['judul']    = 'Data Audit Internal Div. APP Tahun '.$tah. ' dengan Status Monitoring '.$judul;
            }
        }
        $this->load->view('monica/v_excelAuditInternal', $data);
    }

    public function detail_audit_internal(){
        $id_audit        = $this->input->post('id_audit');
        $result_html     = '';
        $result_set      = $this->m_audit->detail_audit_internal($id_audit);
        $today           = date("Y-m-d");
        ...
        foreach($result_set as $result) {
            ...
            $result_html .= '
                <tr>
                    ...
                    <td>' . $result->tanggapan_skai . '</td>
                    <td>' . $result->nama_risk_issue . '</td>
                    <td>' . $result->tipe_temuan . '</td>
                </tr>'; 
        }
        echo json_encode($result_html);
    }

	public function tambah_audit_internal(){
        $tahun                      = substr($this->input->post('sha_code'),-4);
        $sha_code                   = $this->input->post('sha_code');
        ...
        $tanggapan_skai       = $this->input->post('tanggapan_skai');
        ...
        $tipe_temuan          = $this->input->post('tipe_temuan');
        $proses_major         = $this->input->post('proses_major');
        $subproses_major      = $this->input->post('subproses_major');

        if (!empty($_FILES['file']['name'])) {
            $path = './upload/audit/internal/'.$upload_file;
            if(file_exists($path)){
                echo '<script language="javascript">';
                echo 'alert("File yang di upload sudah ada, Data gagal disimpan!!!")';
                echo '</script>';
                redirect('monica/data_audit_internal', 'refresh');
            }else{
        ...
        $simpan = $this->m_audit->tambah_audit_internal($tahun,$sha_code,$tema_audit,$bagian_terkait,$data_temuan,$kategori_temuan,
                  $rekomendasi_1,$status_monitoring_1,$rekomendasi_2,$status_monitoring_2,$rekomendasi_3,$status_monitoring_3,
                  $rekomendasi_4,$status_monitoring_4,$rekomendasi_5,$status_monitoring_5,$rekomendasi_6,$status_monitoring_6,
                  $rekomendasi_7,$status_monitoring_7,$rekomendasi_8,$status_monitoring_8,$rekomendasi_9,$status_monitoring_9,
                  $rekomendasi_10,$status_monitoring_10,$rpm_opt,$tanggapan_skai,$deadline,$tipe,$file_upload,$risk_issue,
                  $tipe_temuan,$proses_major,$subproses_major,$dibuat_oleh,$dibuat_tanggal,$diedit_oleh,$diedit_tanggal);

        if($simpan){
            $upload = $this->upload->do_upload('file');
            echo '<script language="javascript">';
            echo 'alert("Audit internal berhasil ditambahkan")';
            echo '</script>';
            redirect('monica/data_audit_internal', 'refresh');
        }else{
            echo '<script language="javascript">';
            echo 'alert("Audit internal gagal ditambahkan!!!")';
            echo '</script>';
            redirect('monica/data_audit_internal', 'refresh');
        }
    }

    public function update_audit_internal(){
        $id_audit             = $this->input->post('id_audit');
        $sha_code             = $this->input->post('sha');
        $tema_audit           = $this->input->post('tema_audit');
        ...
        if (!empty($_FILES['file']['name'])) {
            $path = './upload/audit/internal/'.$file_upload;
            if(file_exists($path)){
                echo '<script language="javascript">';
                ...
                if(empty($upload_sebelum)){
                    ...
                    $path = './upload/audit/internal/'.$upload_sebelum;
                    unlink($path);
                    $file_upload = $file_upload;
                }
            }
        ...
        for ($i = 0; $i < count($id_audit); $i++) 
        {
            ...
            $risk_issue           = $risk_issue[$id];
            $tipe_temuan          = $tipe_temuan[$id];
            $proses_major         = $proses_major[$id];
            $subproses_major      = $subproses_major[$id];    
            ...
            $update               = $this->m_audit->update_audit_internal($id,$sha_code,$tema_audit,$bagian,
                                    $data_temuan,$kategori,$rekomendasi_1,$status_monitoring_1,$rekomendasi_2,$status_monitoring_2,
                                    ...
                                    $rpm_opt,$tanggapan_skai,$deadline,$file_upload,$risk_issue,$tipe_temuan,$proses_major,
                                    $subproses_major,$diedit_oleh,$diedit_tanggal);
        }

        if($update){
            $upload = $this->upload->do_upload('file');
            echo '<script language="javascript">';
            echo 'alert("Audit internal berhasil diupdate")';
            echo '</script>';
            redirect('monica/data_audit_internal', 'refresh');
        }else{
            echo '<script language="javascript">';
            echo 'alert("Audit internal gagal diupdate!!!")';
            echo '</script>';
            redirect('monica/data_audit_internal', 'refresh');
        }
    }

    public function hapus_audit_internal(){
        $id_audit                    = $this->input->post('id_audit');
        $upload_sebelum              = $this->input->post('upload_sebelum');
        $hapus                       = $this->m_audit->hapus_audit_internal($id_audit);
        if($hapus){
            if(!empty($upload_sebelum)){
                $path_to_file = './upload/audit/internal/'.$upload_sebelum;
                if(file_exists($path_to_file)){
                    unlink($path_to_file);
                }else{}
            }else{}
            echo '<script language="javascript">';
            echo 'alert("Audit internal berhasil dihapus")';
            echo '</script>';
            redirect('monica/data_audit_internal', 'refresh');
        }else{
            echo '<script language="javascript">';
            echo 'alert("Audit internal gagal dihapus!!!")';
            echo '</script>';
            redirect('monica/data_audit_internal', 'refresh');
        }
    }
